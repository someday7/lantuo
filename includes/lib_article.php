...
function get_cat_articles($cat_id, $page = 1, $size = 20 ,$requirement='')
{
    ...
    if ($requirement != '')
    {
        $sql = 'SELECT article_id, title, author, add_time, file_url, open_type,viewnum' .
    ...
    if ($res)
    {
        while ($row = $GLOBALS['db']->fetchRow($res))
        {
            ...
            $arr[$article_id]['viewnum']    = $row['viewnum'];

			$pnum=$GLOBALS['db']->getOne("SELECT count(id) FROM " . $GLOBALS['ecs']->table('replys') . " WHERE article_id = $article_id");

            $arr[$article_id]['pnum']    = $pnum;
        }
    }
    return $arr;
}
