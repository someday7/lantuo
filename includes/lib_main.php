...
function get_user_info($id=0)
{
    ...
    $time = date('Y-m-d');
    $sql  = 'SELECT u.user_id, u.email, u.user_name, u.user_money, u.pay_points,u.avatar'.
            ' FROM ' .$GLOBALS['ecs']->table('users'). ' AS u ' .
            " WHERE u.user_id = '$id'";
...
function assign_ur_here($cat = 0, $str = '')
{
    ...
    $ur_here    = '<a href="."><span class="link-text"><span class="home_icon"></span>' . $GLOBALS['_LANG']['home'] . '</span></a>';
