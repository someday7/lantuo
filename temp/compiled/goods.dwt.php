...
?>
	...
	<div class="pd-side">
		...
		<div class="section1">
			<div class="time">
				<label>收到日期：</label>
				<input class="Wdate" placeholder="请选择日期" type="text" id="d233" name="d233" onFocus="WdatePicker({skin:'whyGreen',dateFmt:'yyyy-MM-dd',minDate:'%y-%M-{%d}',maxDate:'#F{$dp.$D(\'d234\',{d:-0});}'})" />
			</div>
			<div class="time">
				<label>寄回日期：</label>
...
{
  var attr = getSelectedAttributes(document.forms['ECS_FORMBUY']);
  var start_day = document.forms['ECS_FORMBUY'].elements['d233'].value;
  var end_day = document.forms['ECS_FORMBUY'].elements['d234'].value;
//alert(attr + start_day + end_day);
  if(start_day.length>0 && end_day.length>0){
	qty = 1;
	Ajax.call('goods.php', 'act=price&id=' + goodsId + '&attr=' + attr + '&number=' + qty + '&start_day=' + start_day + '&end_day=' + end_day, changePriceResponse2, 'GET', 'JSON');
  }
}
